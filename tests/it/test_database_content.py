...
def test_table_grant_does_not_exist(simple_db_content: DatabaseContent, grant_role: Role) -> None:
    grant_role._safe_create()
    assert not simple_db_content.tables[SimpleTable.__tablename__]._grants_exist(
        grantee=grant_role, privileges={Privilege.SELECT, Privilege.INSERT}
    )


@pytest.mark.order(after="test_table_grant_does_not_exist")
def test_table_grant(simple_db_content: DatabaseContent, grant_role: Role) -> None:
    simple_db_content.tables[SimpleTable.__tablename__].grant(
        grants=[GrantTo(privileges=[Privilege.SELECT, Privilege.INSERT], to=[grant_role])]
    )
...
def test_table_revoke(simple_db_content: DatabaseContent, grant_role: Role) -> None:
